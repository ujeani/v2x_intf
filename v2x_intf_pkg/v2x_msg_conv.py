...
class Parser :
  ...
  def parse(self, pkd_data):
    ...
    self.get_logger().info(f'Header data: {hdr_data}')
    self.get_logger().info(f'--> hdr_flag: {hdr_flag:#X}, msg_recomsg_typenition: {msg_type:#X}, msg_len: {msg_len}')

    if hdr_flag is v2xconst.HDR_FLAG:
      self.get_logger().info('Invalid header flag: %d' % hdr_flag)
      return None
    else :
...
class RecognitionMsg :
  ...
  def fromV2XMsg(self, data): # Header를 제외한 데이터를 수신받아서 Recognition 메시지로 변환
    ...
    self.get_logger().info(f'First part data: {first_part_data}')
    self.get_logger().info(f'--> equipment_type: {equipment_type}, vehicle_time: {vehicle_time}, vehicle_position: {vehicle_position}, positional_accuracy: {positional_accuracy}, num_detected_objects: {num_detected_objects}')
    ...
    for i in range(num_detected_objects):
      ...
      self.get_logger().info(f'Detected object {i}: {detected_object}')
  ...
  def toV2XMsg(self, msg):
    ...
    self.get_logger().info(f'packed_data length : {len(packed_data)}')
